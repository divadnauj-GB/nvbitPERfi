            ...
            volatile int *concurrent_blocks)
{
    ...
    if(threadIdx.x ==0){
        atomicAdd(&(block_index[smid +1]),1);
    }
}
*/
...
main(void)
{
    ...
    vectorAdd<<<blocksPerGrid, threadsPerBlock>>>(d_C, d_B, d_C, numElements);
    ...
    for (int i = 0; i < numElements; ++i)
    {
        if (fabs((h_A[i] + h_B[i] + h_B[i])- h_C[i]) > 1e-5)
        {
