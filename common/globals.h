...
std::ofstream  fout2 ;
//std::string injectionOut = "injection-results.txt";
// std::ofstream  fout3 ;
...
/*void open_injection_file(std::string filename) {
	static bool open_flag = false; // file will be parsed only once - performance enhancement
	if (!open_flag) {
 		fout3.open(filename.c_str(), std::ifstream::out);
		open_flag = true;
	}
}
*/
...
void initInstTypeNameMap() {
	for (int i=0; i<NUM_ISA_INSTRUCTIONS; i++) {
		instTypeNameMap[std::string(instTypeNames[i])] = i;
	}
}
