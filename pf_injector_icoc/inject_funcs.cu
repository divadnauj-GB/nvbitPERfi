...
void inject_error(uint64_t injection_info_ptr, uint64_t verbose_device_ptr,
                  int dest_GPR_num, int reg_val, int num_dest_GPRs, int max_regs, int is_float,
    ...
    if (input_registers_num > 0) {
        r1 = nvbit_read_reg(va_arg(vl, int32_t));
    }
    if (input_registers_num > 1) {
        r2 = nvbit_read_reg(va_arg(vl, int32_t));
    }
    if (input_registers_num > 2) {
        r3 = nvbit_read_reg(va_arg(vl, int32_t));
    }
    if (input_registers_num > 3) {
        r4 = nvbit_read_reg(va_arg(vl, int32_t));
    }
    va_end(vl);

    auto dest_reg_after_val = define_opcode_behavior_32bits(inj_info->instruction_type_out, dest_reg_before_val,
                                                            r1, r2, r3, r4, bool(is_float), verbose_device);
