...
def set_env_variables(): # Set directory paths 
	...
	global report_fname, op_report, profile_name_file, test_list
	...
	test_list = p.script_dir[app] + "/test.list" 

def parse_result():
	#[masked,due,sdc_safe,sdc_critical] = [0, 0, 0, 0]
	masked = list(0 for i in range(0,100))
	due = list(0 for i in range(0,100))
	sdc_safe = list(0 for i in range(0,100))
	sdc_critical= list(0 for i in range(0,100))
	image_list = list (None for i in range(0,100))
	fault_coverage = list(0.0 for i in range(0,100))
	#f = open("/home/jetson/darknet/data/test.list","r")
	f = open(test_list,"r")	
	for i, line in enumerate(f):	
		if(i<100):
			image_list[i] = line.split('/')[7].strip()
	f.close()		
	if os.path.isfile(report_fname):
		...
		for i, line in enumerate(logf):	
				if "code" in line:
					...
					number = line.split('image ')[1].split(',')[0].strip()		
					pos = image_list.index(number)					
		...
		logf.close()
	logf = open(op_report,"w")
	logf.write("Simulation time(h:m): %s\n"%sim_time)
	faults_injected = masked[0]+due[0]+sdc_safe[0] + sdc_critical[0]
	logf.write("Total Faults injected: %s\n"%faults_injected)
	logf.write("Image  Faults classification: Masked     DUE     SDC: sdc_safe sdc_critical    fault_coverage \n") 
	for i in range(0,100):
		fault_coverage[i] = float(sdc_safe[i] + sdc_critical[i])/float(masked[i]+due[i]+sdc_safe[i] + sdc_critical[i])
		formatted_float = "{:.2%}".format(fault_coverage[i])
		print ("Image %s -> Faults classification: masked %s, due %s, SDC: sdc_safe %s, sdc_critical %s, fault_coverage %s" %(image_list[i],masked[i],due[i],sdc_safe[i],sdc_critical[i],formatted_float))
		logf.write("%-*s %-*s %-*s %-*s %-*s %-*s\n" %(30,image_list[i],9,masked[i],13,due[i],10,sdc_safe[i],15,sdc_critical[i],7,formatted_float))
